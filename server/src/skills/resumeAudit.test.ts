...
assert.equal(audit.checks.find((check) => check.id === "R006")?.passed, false);
assert.equal(audit.checks.find((check) => check.id === "R006")?.name, "表达优化");
assert.equal(audit.checks.find((check) => check.id === "R006")?.severity, "suggestion");
